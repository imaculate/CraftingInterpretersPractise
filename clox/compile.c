...
static void freeCompiler(Compiler* compiler)
{
    ...
    initCompiler(compiler, compiler->type);
}
...
{
    return makeConstant(OBJ_VALUE(copyString(name->start, name->length)));
}
...
static void addLocal(Token name)
{
    ...
    if (current->localCapacity <= current->localCount)
    {
...
{
    emitConstant(OBJ_VALUE(copyString(parser.previous.start + 1, parser.previous.length - 2)));
}
...
{
    advance();
    // printf("Parsing parser token:  %d \n", parser.previous.type); // debug code
